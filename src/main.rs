...
#[derive(Parser, Debug)]
#[clap(author="MGTheTrain", version="1.0.0", about="A Cli tool enabling blob operations (deletion, upload and download of blobs) in an Azure Storage Account Container.")]
struct Cli {
    ...
    match &args.operation {
        AzureStorageAccountContainerOperation::Upload { upload_file_path, blob_name } => {
            ...
            let client = ClientBuilder::new(
                account, storage_credentials).blob_client(container_name, blob_name.clone().unwrap());
